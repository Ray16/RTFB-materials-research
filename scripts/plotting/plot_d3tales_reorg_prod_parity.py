...
def main():
    ...
    apply_style()
    panels = [
        ("our_smd_eV", "d3tales_eV", "our SMD-opt  vs  D3TaLES", "D3TaLES reported $\\lambda$ (eV)", "our SMD-opt $\\lambda$ (eV)"),
        ("our_b3lyp_gas_eV", "d3tales_eV", "our B3LYP-gas  vs  D3TaLES", "D3TaLES reported $\\lambda$ (eV)", "our B3LYP-gas $\\lambda$ (eV)"),
        ("our_smd_eV", "our_b3lyp_gas_eV", "our SMD-opt  vs  our B3LYP-gas", "our B3LYP-gas $\\lambda$ (eV)", "our SMD-opt $\\lambda$ (eV)"),
    ]
    fig, axes = plt.subplots(1, 3, figsize=(19, 6.4))
    for ax, (xk, yk, title, xl, yl) in zip(axes, panels):
        pts = [(_f(r[xk]), _f(r[yk]), _famcolor(r["family"])) for r in rows]
    ...
    fig.suptitle("Inner-sphere $\\lambda$ (electron couple): the D3TaLES gap survives solvent removal",
